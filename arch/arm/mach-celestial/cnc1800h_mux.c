...
#define  CSM18XX_PINMUX_BASE 0x42110000
#define  CSM18XX_PINMUX_SIZE 0x100
...
static struct class *csm120x_pinmux_class;
enum pinmux_class  mode;
// should use in higher kernel version:static struct class *csm120x_pinmux_class;
/*
 * The devices
 */
...
static int __init csm18xx_pinmux_init(void);
static void __exit csm18xx_pinmux_exit(void);

static int csm18xx_pinmux_open(struct inode *inode, struct file *file);
static int csm18xx_pinmux_close (struct inode *inode, struct file *file);
...
struct file_operations pinmux_fops = {
	.owner = THIS_MODULE,
	.open = csm18xx_pinmux_open,
	.release  = csm18xx_pinmux_close,
};



int csm_gpio_register_module_set(const char * module, unsigned int  csm_module_status) /* status: 0 disable, 1 enable */
{
	if(strncmp("RC", module, 2)==0){ 
					if (csm_module_status == 1){
						gpio_configure_status_bit(11 ,"SET",1);
					}
					if (csm_module_status == 0){
 	...
 	}else if (strncmp("DVIO", module, 4)==0){
 					if (csm_module_status == 1){
						gpio_configure_status_bit(32,"SET",1);
						gpio_configure_status_bit(33,"SET",1);
						gpio_configure_status_bit(34,"SET",1);
						gpio_configure_status_bit(35,"SET",1);
						gpio_configure_status_bit(36,"SET",1);
						gpio_configure_status_bit(37,"SET",1);
						gpio_configure_status_bit(38,"SET",1);
						gpio_configure_status_bit(39,"SET",1);
						gpio_configure_status_bit(40,"SET",1);						
	 				}
					if (csm_module_status == 0){
						gpio_configure_status_bit(32,"CLR",1);
						gpio_configure_status_bit(33,"CLR",1);
						gpio_configure_status_bit(34,"CLR",1);
						gpio_configure_status_bit(35,"CLR",1);
						gpio_configure_status_bit(36,"CLR",1);
						gpio_configure_status_bit(37,"CLR",1);
						gpio_configure_status_bit(38,"CLR",1);
						gpio_configure_status_bit(39,"CLR",1);
						gpio_configure_status_bit(40,"CLR",1);	
	 				}
 	}else {
		return 0;
	}
...
printk("mode  = %0x ,bit = %0x,set = %0x\n",mode,bit,set);
		...
		switch(mode){
			...
			case PINMUX_DISP:
				...
				if(bit <= 5){
						if(set ){
								//int bitmap1 = pinmux_get_status_bit( mode,9 );
								//int bitmap2 = pinmux_get_status_bit( mode,10 );
								//int bitmap3 = pinmux_get_status_bit( mode,11 );
								if(bit == TVE0_YPbPr){
			...
			case PINMUX_DVIO:
				if(bit == 2){
					if(set)pinmux_base[REG_PINMUX_DVIO>>1] |= (1<<(bit-1));
...
int pinmux_enable_gpio(GPIO_ID GPIO)
{
    ...
    switch(GPIO)
    {
        ...
        case _gpio_1     :
			...
			csm_gpio_register_module_set("I2C1",0);
            break;
        case _gpio_2     :
            break;
        case _gpio_3     :
		case _gpio_6     :
			...
			csm_gpio_register_module_set("URAT2",0);
            break;
        case _gpio_4     :
            break;
        case _gpio_5     :
        ...
        case _gpio_17    :
			...
			csm_gpio_register_module_set("I2C0",0);
            break;	
        case _gpio_18    :
        case _gpio_19    :
			pinmux_base[REG_PINMUX_UART>>1] &= ~1;
			csm_gpio_register_module_set("URAT0",0);
            break;
        case _gpio_20    :
			printk("has not used \n");
            break;
        ...
        case _gpio_26    :
			...
			csm_gpio_register_module_set("SPI",0);
            break;
        case _gpio_27    :
        case _gpio_28    :
        case _gpio_29    :
        ...
        case _gpio_31    :
			...
			csm_gpio_register_module_set("SMI",0);
            break;
        case _gpio_32    :
        case _gpio_33    :
        case _gpio_34    :
        ...
        case _gpio_40    :
			...
			csm_gpio_register_module_set("DVIO",0);
            break;
        default:
            ;
...
int pinmux_enable_i2c(I2C_ID I2C)
{
	...
	switch(I2C)
	{
		case _i2c_0:
			pinmux_base[REG_PINMUX_I2C>>1] |= 1;
			csm_gpio_register_module_set("I2C0",1);
			break;
		case _i2c_1:
			pinmux_base[REG_PINMUX_I2C>>1] |= (1<<1);
			pinmux_base[REG_PINMUX_I2C>>1] &= ~(1<<2);
			csm_gpio_register_module_set("I2C1",1);
			break;
		default:
			;
	}
...
int pinmux_enable_uart(UART_ID UART)
{
	...
	switch(UART)
	{
		case _uart_0:
			pinmux_base[REG_PINMUX_UART>>1] |= 1;
			csm_gpio_register_module_set("URAT0",1);
			break;
		case _uart_1:
			pinmux_base[REG_PINMUX_UART>>1] |= (1<<5);
		...
		case _uart_2:
			...
			csm_gpio_register_module_set("URAT2",1);
			break;
		case _uart_audio:
			pinmux_base[REG_PINMUX_UART>>1] |= (1<<1);
			pinmux_base[REG_PINMUX_UART>>1] |= (1<<3);
...
{
	pinmux_base[REG_PINMUX_SPI>>1] |= 1;
	csm_gpio_register_module_set("SPI",1);
	return 0;
}
...
int pinmux_enable_smi(void)
{
	...
	pinmux_base[REG_PINMUX_SMI>>1] |= 1;
	csm_gpio_register_module_set("SMI",1);
	return 0;
...
int pinmux_enable_vib(void)
{
	...
	pinmux_base[REG_PINMUX_DVIO>>1] |= (1<<1);
	csm_gpio_register_module_set("DVIO",1);
	return 0;
}
...
int pinmux_update_rc_status(void)
{
    ...
    //    rc_ctl = rc_ctl >> 15;
    csm_gpio_register_module_set("RC", rc_ctl?1:0);  
 	return 0;
}
...
int pinmux_update_fpc_status(void)
{
    ...
    csm_gpio_register_module_set("FPC-COMMON", arg);

    csm_gpio_register_module_set("FPC-LED-SPECIAL", led_ctl? 1: 0);
...
//EXPORT_SYMBOL(csm_gpio_register_module_set);

static int
csm18xx_pinmux_open(struct inode *inode, struct file *file)
{
	unsigned m = iminor(file->f_dentry->d_inode);
	printk("csm18xx_pinmux_open %0x \n",m);
	return 0;
}

static int 
csm18xx_pinmux_close (struct inode *inode, struct file *file)
{
	unsigned m = iminor(file->f_dentry->d_inode);
	printk("csm18xx_pinmux_close %0x \n",m);
	return 0;
}
...
static int  pinmux_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	ret = pinmux_base[REG_PINMUX_DVIO>>1];
		if(ret & 0x2)
			len += sprintf(page+len,"==> REG_PINMUX_TS's PINMUX %d is held by digital video input . \n",i);
		else
			len += sprintf(page+len,"==> REG_PINMUX_TS's PINMUX %d is held by GPIO[40:32]. \n",i++);
	ret = pinmux_base[REG_PINMUX_ETHER>>1];
		if(ret & 0x1)
			len += sprintf(page+len,"==> REG_PINMUX_TS's PINMUX %d  ehternet disabled and boot config parameter input enable. \n",i);
		else
			len += sprintf(page+len,"==> REG_PINMUX_TS's PINMUX %d is held by Ethernet . \n",i++);
...
int csm18xx_pinmux_init_setting(void)
{
...
csm18xx_pinmux_init(void)
{
	...
	if (!request_mem_region(CSM18XX_PINMUX_BASE, CSM18XX_PINMUX_SIZE, "CSM18XX PINMUX")) {
		unregister_chrdev_region(dev, pinmux_nr_devs);
		return -EIO;
	}

	pinmux_base =
	    (volatile unsigned short *) ioremap(CSM18XX_PINMUX_BASE,
						CSM18XX_PINMUX_SIZE);
	if (!pinmux_base) {
		unregister_chrdev_region(dev, pinmux_nr_devs);
    ...
    csm120x_pinmux_class = class_create(THIS_MODULE,"pinmux");
	device_create(csm120x_pinmux_class, NULL, MKDEV(pinmux_major, 0) , NULL, "pinmux");

    if (IS_ERR(csm120x_pinmux_class)){
        err = PTR_ERR(csm120x_pinmux_class);
		unregister_chrdev_region(dev, pinmux_nr_devs);
	...
	if (!pinmux_devices) {
		...
		release_mem_region(CSM18XX_PINMUX_BASE, CSM18XX_PINMUX_SIZE);
		unregister_chrdev_region(dev, pinmux_nr_devs);
		return -ENOMEM;
	}
	...
	printk(KERN_INFO "CSM18XX PINMUX at 0x%x, %d lines\n", CSM18XX_PINMUX_BASE,
	       pinmux_nr_devs);
    ...
    csm18xx_pinmux_init_setting();

	return 0;
}

static void __exit
csm18xx_pinmux_exit(void)
{
	int i;
...
module_init(csm18xx_pinmux_init);
module_exit(csm18xx_pinmux_exit);

MODULE_AUTHOR("Celestial Semiconductor");
MODULE_DESCRIPTION("Celestial Semiconductor PINMUX driver");
MODULE_LICENSE("GPL");
