...
static DISP_PLL_CFG disp_pll_cfg[] = {
	...
	{_RGB_CVT_1366x768_60Hz,70200000,0,5,25},
};
...
void clock_ddcset_clockmode(DDC_CLOCK_MODE ClockMode)
{
    ...
    iDDCDiv =0;//16/5
    reg_val = df_clk_read(CLOCK_CLK_GEN_DIV);
    df_clk_write(CLOCK_CLK_GEN_DIV, reg_val&(~(0x1f<<16)));

    switch (ClockMode) {
    case _ddc_clk_high:
        iDDCDiv =10;    //70K
        reg_val = df_clk_read(CLOCK_CLK_GEN_DIV);
        df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|(0xA<<16));
        break;
    case _ddc_clk_middle:   //60K
        iDDCDiv =12;
        reg_val = df_clk_read(CLOCK_CLK_GEN_DIV);
        df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|(0xC<<16));
        break;
    case _ddc_clk_low:     //50K
        iDDCDiv =14;
        reg_val = df_clk_read(CLOCK_CLK_GEN_DIV);
        df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|(0xE<<16));
        break;
    default:
        ;
    }
...
void clock_cecset_clockmode(Cec_CLOCK_MODE ClockMode)
{
	...
	switch (ClockMode){
		case _Cec_94P5M:
		df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|0x3);
		break;
		case _Cec_2P1M:
		df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|0xb3);
		break;
		case _Cec_1P48M:
		df_clk_write(CLOCK_CLK_GEN_DIV, reg_val|0xfe);
		break;
	}
...
void clock_audio_setclock(AUDIO_CLOCK_MODE AudioClockMode)
{
    ...
    if (SleepOrNot) {
        ...
        //clock_audio_clockena(_clock_disable);	// lixun: for audio noise
        ...
        clock_audio_clockena(_clock_enable);
...
void clock_audio_clockena(CLOCK_ENA ClockEna)
{
    ...
    if (ClockEna == _clock_enable) {
        iAudioClkEna = 1;//8/1
        reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
        df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(0x1<<22));
    } else {
        iAudioClkEna = 0;
        reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
        df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<22)));
...
void power_usb0phy_down(POWER_MODE PowerMode)
{
...
void power_usb1phy_down(POWER_MODE PowerMode)
{
    int iUSBPhyAnlogBlockPD;
    unsigned int reg_val;
...
void clock_xport_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<21)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<21));
	}
...
void clock_vib_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<28)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<28));
	}
...
void clock_aud_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<22)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<22));
	}
...
void clock_crypto_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<20)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<20));
	}
...
void clock_graphics_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<18)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<18));
	}
...
void clock_df_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<17)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<17));
	}
...
void clock_videcode_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<16)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<16));
	}
...
void clock_auddacif_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<9)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<9));
	}
...
void clock_audpll_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<8)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<8));
	}
...
void clock_sd_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<7)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<7));
	}
...
void clock_tve1_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<5)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<5));
	}
...
void clock_tve0_down(POWER_MODE PowerMode)
{
	unsigned int reg_val;
	
	if (PowerMode == _clock_sleep){
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val&(~(0x1<<4)));
	}else{
		reg_val = df_clk_read(CLOCK_LOW_POWER_CTL);
		df_clk_write(CLOCK_LOW_POWER_CTL, reg_val|(0x1<<4));
	}
...
void power_audio_on(POWER_MODE PowerMode)
{
    unsigned int reg_val;

    if (PowerMode==_clock_wake) {
        reg_val = df_clk_read(CLOCK_AUD_POWER_ON);
        df_clk_write(CLOCK_AUD_POWER_ON, (reg_val|0x1));
    } else {
        reg_val = df_clk_read(CLOCK_AUD_POWER_ON);
        df_clk_write(CLOCK_AUD_POWER_ON, reg_val&(~(0x1)));
    }
...
void clock_video_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<23));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<23)));
	}
...
void clock_vib_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<28));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<28)));
	}
...
void clock_coda_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<16));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<16)));
	}
...
void clock_gfx_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<18));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<18)));
	}
...
void clock_crypto_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<20));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<20)));
	}
...
void clock_tms_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(1<<6));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<6)));
...
void clock_cec_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(0x1<<26));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<26)));
...
void clock_df_clockena(CLOCK_ENA ClockEna)
{
	...
	if (ClockEna == _clock_enable) {
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val|(0x1<<17));
	}
	else{
		reg_val = df_clk_read(CLOCK_PLL_CLK_EN);
		df_clk_write(CLOCK_PLL_CLK_EN, reg_val&(~(0x1<<17)));
...
unsigned int clock_read_fixme(unsigned int idcs_reg_addr)
{
	return df_clk_read(idcs_reg_addr);
}
EXPORT_SYMBOL( clock_read_fixme);
...
EXPORT_SYMBOL( clock_gfx_reset);
EXPORT_SYMBOL( clock_video_reset);
EXPORT_SYMBOL( clock_crypto_reset);
EXPORT_SYMBOL( clock_usb_reset);
EXPORT_SYMBOL( clock_ether_reset);
EXPORT_SYMBOL( clock_xport_reset);

EXPORT_SYMBOL( clock_audio_clockena);
EXPORT_SYMBOL( clock_video_clockena);
EXPORT_SYMBOL( clock_vib_clockena);
EXPORT_SYMBOL( clock_coda_clockena);
EXPORT_SYMBOL( clock_gfx_clockena);
EXPORT_SYMBOL( clock_crypto_clockena);
EXPORT_SYMBOL( power_audio_on);
//EXPORT_SYMBOL( clock_ddc_clockena);
EXPORT_SYMBOL( clock_cec_clockena);
EXPORT_SYMBOL( clock_tms_clockena);
EXPORT_SYMBOL( clock_df_clockena);
static int cs_pwr_ioctl(struct inode *inode, struct file *file,
	unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case USB_POWER_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			power_usb0phy_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case XPORT_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_xport_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case VIB_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
		...
		case AUD_CLK_DOWN:
			...
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case CRYPTO_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_crypto_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case GRAPHICS_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_graphics_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case DF_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			...
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case VIDECOD_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_videcode_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case AUDDACIF_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_auddacif_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case SD_CLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
		...
		case VID_POWER_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_dispset_dac0mode(arg);
			clock_dispset_dac1mode(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case AUD_PLLCLK_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_audpll_down(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case VDAC0_POWER_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_dispset_dac0mode(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
		case VDAC1_POWER_DOWN:
			spin_lock_irqsave(cnc_pwr_lock, flags);
			clock_dispset_dac1mode(arg);
			spin_unlock_irqrestore(cnc_pwr_lock, flags);
			break;
...
static struct file_operations pwr_fops = {
	.owner		= THIS_MODULE,
	.ioctl		= cs_pwr_ioctl,
};

static struct miscdevice pwr_miscdev = {
	.name = "cs_pwr",
			   ...
			   unsigned long count, void *data)
{
	...
	if (strncmp("usb0", &cmd_line[it], 4) == 0) {
        ...
        if (strncmp("up", &cmd_line[it], 2) == 0){
            it += 4;
            power_usb1phy_down(_clock_wake);        
            printk("USB1 PHY power is up!\n");
        }else if (strncmp("down", &cmd_line[it], 4) == 0){
            it += 4;
            power_usb1phy_down(_clock_sleep);        
        ...
        funs_name[15] = "clock_audio_clockena(CLOCK_ENA ClockEna)";
        ...
        funs_name[29] = "unused";
        funs_name[30] = "unused";
        funs_name[31] = "clock_crypto_reset(CLOCK_RESET ResetOrSet)";
        funs_name[32] = "clock_video_reset(CLOCK_RESET ResetOrSet)";
        funs_name[33] = "clock_gfx_reset(CLOCK_RESET ResetOrSet)";
...
static int __init
cnc1800l_clock_init(void)
{
	if (NULL == df_clk_base) {
		if (!(df_clk_base = (unsigned char *)ioremap(CLOCK_REG_BASE, CLOCK_REG_SIZE))) {
			printk("ERROR: cnc1800l_clock_init ioremap failed\n");
			return (-1);
		} 
		else{
			printk("CNC1800L_CLOCK init at %0x\n",CLOCK_REG_BASE);
		}
	}

	if (misc_register(&pwr_miscdev)) {
	...
	if (NULL != power_proc_entry) {
		...
		iounmap((void *)df_clk_base);
		misc_deregister(&pwr_miscdev);
		printk("ERROR: cnc1800l_clock_init create_proc_entry failed\n");
		return (-1);
	}
...
static void __exit
cnc1800l_clock_exit(void)
{
	iounmap((void *)df_clk_base);
	misc_deregister(&pwr_miscdev);

	if (NULL != power_proc_entry){
		remove_proc_entry("powerctl", NULL);
	}

	return;
}

module_init(cnc1800l_clock_init);
module_exit(cnc1800l_clock_exit);

MODULE_AUTHOR("Cavium Celestal SDK team");
MODULE_DESCRIPTION("Celestial  CLOCK driver");
MODULE_LICENSE("GPL");
