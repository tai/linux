...
#include "video_reg_def.h"
#include "csm1800_video.h"
...
static volatile unsigned int *vid_reset_base = NULL;
...
//			printk("mips start\n");
		...
		case CSVID_IOC_RELOAD_FIRMWARE:
			...
			__video_init(0);
			//udelay(100);
			video_write(0x0, HOST_IF_VID_MIPS_MAILBOX_5);
			video_write(0x0, HOST_IF_VID_MIPS_MAILBOX_7);
...
static int __video_init(int load_coda_flag)
{
	...
	video_write(0x0, HOST_IF_VID_MIPS_STA1);
#if 0
...
static int __csm_video_release_firmware(void)
{
	...
	for (ch = 0; ch < MAX_CHANNEL_NUM; ch++){
		...
		if (host2videofw_if->channel_num & (1 << ch)){ // current channel enable, send stop cmd 
			i = 0;
			while (video_read(HOST_IF_VID_MIPS_MAILBOX_2) >> 31){ // wait last cmd ack
				i++;
				udelay(100);
			...
			video_write(cmd|0x80000000, HOST_IF_VID_MIPS_MAILBOX_2);

			do{
				if((video_read(HOST_IF_VID_MIPS_MAILBOX_2) >> 31) == 0x0)
					break;
	...
	// close interrupt
	reg = *(vidint_base + HOST_IF_VID_HOST_INT_ENABLE);
	//    printk("HOST_INT_ENABLE = 0x%08x\n", reg);
	reg |= (1 << 31);
	*(vidint_base + HOST_IF_VID_HOST_INT_ENABLE) = reg;
	//    printk("HOST_INT_ENABLE = 0x%08x\n", reg);
	*(vidint_base + HOST_IF_VID_HOST_INT_FORCE) = (1 << 31);

	i = 0;
	do{
		reg = *(vidint_base + HOST_IF_VID_HOST_INT_FORCE);
	...
	{ // for test
		...
		printk("vid_reset_base =0x%08x reset_reg = 0x%08x \n", (unsigned int)vid_reset_base, (unsigned int )(vid_reset_base + HOST_IF_VID_RESET));
...
{
	return __csm_video_release_firmware();
}
...
{	
	...
	unsigned int channel_id = 0;
		...
		printk("csm_vid_irq!\n");
#endif
	irq_reg = video_read(HOST_IF_VID_MIPS_MAILBOX_4);
#ifdef DRV_VIDEO_DEBUG
		printk("irq_reg = 0x%x\n",irq_reg);
#endif
...
#ifdef DRV_VIDEO_DEBUG
		printk("vid_dint_format = 0x%x(stc = 0x%08x)\n",host2videofw_if->video_channel_para[channel_id].vid_dint_format, video_read(HOST_IF_VID_STC));
#endif
...
#ifdef DRV_VIDEO_DEBUG
		printk("vid_aspect_ratio = 0x%x(stc = 0x%08x)\n",host2videofw_if->video_channel_para[channel_id].vid_aspect_ratio, video_read(HOST_IF_VID_STC));
#endif
                spin_unlock_irqrestore(&notify_lock,flag);
       }
...
#ifdef DRV_VIDEO_DEBUG
		printk("vid_src_format = 0x%x(stc = 0x%08x)\n",host2videofw_if->video_channel_para[channel_id].vid_src_format, video_read(HOST_IF_VID_STC));
#endif
		spin_unlock_irqrestore(&notify_lock,flag);
	}
...
//		printk("reset step 2 done\n");
		...
		video_write(0xf0f0f0f0, HOST_IF_VID_MIPS_MAILBOX_31);
//		printk("reset coda done  = 0x%08x\n", video_read(HOST_IF_VID_STC));
	}

	if(irq_reg&VID_INT_DECODE_FINISH){
#ifdef DRV_VIDEO_DEBUG
		printk("VID_INT_DECODE_FINISH!\n");
#endif
		spin_lock_irqsave(&notify_lock,flag);
		if(vid_flags.bits.decode_finished_status){
			vid_flags.bits.decode_finished_irq = 1;
			wake_up(&notify_queue);
		}
		spin_unlock_irqrestore(&notify_lock,flag);
	}
	...
	{
		unsigned int temp_val = 0;
		temp_val = video_read(HOST_IF_VID_MIPS_MAILBOX_4);
		temp_val = temp_val^irq_reg;
		video_write(temp_val, HOST_IF_VID_MIPS_MAILBOX_4);
	}

	*(vidint_base + (HOST_IF_VID_HOST_INT)) = 0;
	udelay(10);
...
{
	if (!request_mem_region(CSM_VIDEO_REG_BASE, CSM_VIDEO_REG_SIZE, "CSM Video"))
		goto ERR_MEM1;
	...
	if (!request_mem_region(CSM_VIDEO_INT_BASE, CSM_VIDEO_INT_SIZE, "Video INT"))
		goto ERR_MEM5;

	if (NULL == (vidint_base = ioremap(CSM_VIDEO_INT_BASE, CSM_VIDEO_INT_SIZE)))
		goto ERR_MEM6;

	if (!request_mem_region(CSM_VIDEO_RESET_BASE, CSM_VIDEO_RESET_SIZE, "Video RESET"))
                goto ERR_MEM7;

        if (NULL == (vid_reset_base = ioremap(CSM_VIDEO_RESET_BASE, CSM_VIDEO_RESET_SIZE)))
                goto ERR_MEM8;

	return;
	
	ERR_MEM8:
        release_mem_region(CSM_VIDEO_RESET_BASE, CSM_VIDEO_RESET_SIZE);
        ERR_MEM7:
        iounmap((void *) vid_reset_base);
	ERR_MEM6:
	release_mem_region(CSM_VIDEO_INT_BASE, CSM_VIDEO_INT_SIZE);
	ERR_MEM5:
...
#define DIRWPRP_SIZE (0x40) // 4*2*8
static int __init csm_video_init(void)
{
	if (register_chrdev(VIDEO_MAJOR, "csm_video", &csm_video_fops)){
		return -ENODEV;
	}
	...
	csm_video_class = class_create(THIS_MODULE,"csm_video");
	...
	device_create(csm_video_class, NULL, MKDEV(VIDEO_MAJOR, 0), NULL, "csm_video");
	...
	if (request_irq(20, csm_vid_irq, 0, "cs_video", NULL)) {
...
//	host2videofw_if = (_HOST_FW_IF_t *)(vidmcu_base + VIDEO_HOST_OFFSET);
	...
	printk("CSM1800 Video System Version : 0.2\n");
...
static void __exit csm_video_exit(void)
{
	...
	unregister_chrdev(VIDEO_MAJOR, "csm_video");

	printk(KERN_INFO " CSM VIDEO Exit ...... OK. \n");
