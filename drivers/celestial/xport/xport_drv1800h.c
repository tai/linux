...
static int xport_hw_init(void)
{
	...
	xport_writel(IN_CHL2_BASE_ADDR, (IN_CHL2_BASE_ADDR_DEF >> 3));
	xport_writel(IN_CHL2_CFG_ADDR, IN_CHL2_CFG_DEF);
	xport_writel(IN_CHL2_CFG_ADDR, IN_CHL2_CFG_DEF);

	xport_writel(IN_CHL3_BASE_ADDR, (IN_CHL3_BASE_ADDR_DEF >> 3));
	xport_writel(IN_CHL3_CFG_ADDR, IN_CHL3_CFG_DEF);
	xport_writel(IN_CHL3_CFG_ADDR, IN_CHL3_CFG_DEF);
...
#else
		xport_dev->irq_wait_queue_ptr = (chl_id%2?(chl_id==1?&irq0_dma1_wait_queue:&irq0_dma3_wait_queue):
				(chl_id==0?&irq0_dma0_wait_queue:&irq0_dma2_wait_queue));
#endif
...
static unsigned int xport_poll(struct file *file, poll_table * wait)
{
	...
	if(xport_dev->dev_minor == XPORT_MINOR_CHL(0)
			|| xport_dev->dev_minor == XPORT_MINOR_CHL(2))
	{
		if (xport_dma_input_check(0))
			...
			|| xport_dev->dev_minor == XPORT_MINOR_CHL(3))
	{
	...
	else if (__IS_MIPS_ADDR__(cmd))
	{
		...
		switch (cmd)
		{
			...
			case XPORT_CHL_IOC_DES_MODE:
				...
				mnr = (mnr%2==0?(mnr==XPORT_MINOR_CHL(0)?0:2):(mnr==XPORT_MINOR_CHL(1)?1:3));
			...
			case XPORT_CHL_IOC_TUNER_MODE:
				...
				if(mnr == XPORT_MINOR_CHL(2) || mnr == XPORT_MINOR_CHL(3))	{
					//printk("wrong input channel:only chl0&chl1 surpport tuner input \n");
					break;
				}
			...
			case XPORT_CHL_IOC_INPUT_MODE:
				...
				tmp_val = xport_readl(XPORT_CFG_ADDR0);
#ifdef CONFIG_MACH_CELESTIAL_CNC1800L
                mask_val = ((mnr-XPORT_MINOR_CHL(0))%2 ?
                        (0x100<<(8*(mnr-XPORT_MINOR_CHL(1)))):(0x1<<(8*(mnr-XPORT_MINOR_CHL(0)))) );
#else
				mask_val = ((mnr-XPORT_MINOR_CHL(0))%2 ?
						(0x300<<(8*(mnr-XPORT_MINOR_CHL(1)))):(0x1<<(8*(mnr-XPORT_MINOR_CHL(0)))) );
#endif
				...
				switch (regs_val)
				{
					...
					case 2:	/* direct */
						...
						if(mnr == XPORT_MINOR_CHL(1))
							tmp_val |= (XPORT_IRQ0_DMA1_EMPTY_MSK);
						if(mnr == XPORT_MINOR_CHL(2))
							tmp_val |= (XPORT_IRQ0_DMA2_EMPTY_MSK);
						if(mnr == XPORT_MINOR_CHL(3))
							tmp_val |= (XPORT_IRQ0_DMA3_EMPTY_MSK);
						break;
				}
			...
			case XPORT_CHL_IOC_MIN_SPACES:
				if (NULL != (void *)arg){
					...
					else if (xport_dev->dev_minor == XPORT_MINOR_CHL(2))
						tmp_val = XPORT_CHL2_MIN_SPACES;
					else if (xport_dev->dev_minor == XPORT_MINOR_CHL(3))
						tmp_val = XPORT_CHL3_MIN_SPACES;
					else
						return -EINVAL;
					copy_to_user((void __user *)arg, &tmp_val, sizeof(unsigned int));
				}
            ...
            case XPORT_IOC_DMA0_MAX_BLOCK_NUM:
                ...
                break;
            case XPORT_IOC_DMA1_LIST0_HEAD_ADDR:
                __put_user(DMA1_LIST0_HEAD_ADDR, (int __user *)arg);
                break;
            case XPORT_IOC_DMA1_BUF0_BASE_ADDR:
                __put_user(DMA1_BUF0_BASE_ADDR, (int __user *)arg);
                break;
            case XPORT_IOC_DMA1_LIST1_HEAD_ADDR:
                __put_user(DMA1_LIST1_HEAD_ADDR, (int __user *)arg);
                break;
            case XPORT_IOC_DMA1_BUF1_BASE_ADDR:
                __put_user(DMA1_BUF1_BASE_ADDR, (int __user *)arg);
                break;
            case XPORT_IOC_DMA1_MAX_BLOCK_SIZE:
                __put_user(DMA1_MAX_BLOCK_SIZE, (int __user *)arg);
                break;
            case XPORT_IOC_DMA1_MAX_BLOCK_NUM:
                __put_user(DMA1_MAX_BLOCK_NUM, (int __user *)arg);
                break;
		}
...
static void xport_show_buffer_define(void)
{
	...
	printk("Xport DMA1 Buf0 Base: 0x%x\n", DMA1_BUF0_BASE_ADDR);
	printk("Xport DMA1 Buf0 Size: 0x%x\n", DMA1_BUF0_SIZE);
	printk("Xport DMA1 Buf1 Base: 0x%x\n", DMA1_BUF1_BASE_ADDR);
	printk("Xport DMA1 Buf1 Size: 0x%x\n", DMA1_BUF1_SIZE);
	printk("Xport DMA1 List0 Head Base: 0x%x\n", DMA1_LIST0_HEAD_ADDR);
	printk("Xport DMA1 List0 Head Size: 0x%x\n", DMA_LIST_HEAD_SIZE);
	printk("Xport DMA1 List1 Head Base: 0x%x\n", DMA1_LIST1_HEAD_ADDR);
	printk("Xport DMA1 List1 Head Size: 0x%x\n", DMA_LIST_HEAD_SIZE);
