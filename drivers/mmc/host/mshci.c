...
static void mshci_reset_dma(struct mshci_host *host)
{
    ...
    /*
     * 2012-08-21 added by xp, DMA_RESET bit in CTRL register just reset DMA interface,
     * need to use SWR bit in BMOD register to reset all internal DMA registers.
     */
    mshci_writel(host, 1, MSHCI_BMOD);
    mdelay(1);
}
...
#else
    mshci_writel(host, 0x20100010, MSHCI_FIFOTH);
#endif
}
...
int mshci_add_host(struct mshci_host *host)
{
	...
	mmc->max_segs = mmc->max_phys_segs;
